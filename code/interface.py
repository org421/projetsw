#!/usr/bin/env python3
"""
Tolkien Knowledge Graph - Linked Data Interface
Interface Web pour naviguer dans les données RDF via Flask & SPARQLWrapper.
"""
...
# Adresse de ton serveur Fuseki (Vérifie bien le nom du dataset 'tolkien')
FUSEKI_ENDPOINT = "http://localhost:3030/tolkien/query"

# Espaces de noms (Doivent correspondre à ton RDFGenerator)
NAMESPACES = {
...
# Types d'infobox pour le menu déroulant
INFOBOX_TYPES = [
...
def query_entity_description(entity_uri: str) -> list:
    """Récupère tous les triplets liés à une entité (entrants et sortants)."""
    sparql = get_sparql()
    
    query = f"""
    {SPARQL_PREFIXES}
    SELECT DISTINCT ?predicate ?object ?isIncoming
    WHERE {{
        {{
            SELECT ?subject WHERE {{
                {{ BIND(<{entity_uri}> AS ?subject) }}
                UNION
                {{ <{entity_uri}> owl:sameAs ?subject }}
                UNION
                {{ ?subject owl:sameAs <{entity_uri}> }}
            }}
        }}
...
def query_all_classes_with_superclasses(entity_uri: str) -> list:
    """Récupère les types et super-types (Inférence)."""
    sparql = get_sparql()
    query = f"""
    {SPARQL_PREFIXES}
...
def query_card_data_for_entity(entity_uri: str) -> list:
    """Récupère les données des cartes MECCG liées."""
    sparql = get_sparql()
    query = f"""
    {SPARQL_PREFIXES}
...
def generate_turtle_fallback(entity_uri: str) -> str:
    """Génère un TTL basique si la requête CONSTRUCT échoue."""
    ...
    if properties:
        ...
        lines.append("    a owl:Thing .")
    
    return "\n".join(lines)


def query_entity_turtle(entity_uri: str) -> str:
    """Génère le format Turtle via SPARQL CONSTRUCT."""
    sparql = get_sparql()
    query = f"""
    {SPARQL_PREFIXES}
...
def categorize_uri(uri: str) -> str:
    ...
    elif "yago" in uri: return "yago"
    elif "MECCG" in uri: return "card"
    return "wiki"
...
def parse_accept_header(accept: str) -> str:
    ...
    if "text/turtle" in accept.lower(): return "turtle"
    return "html"
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .lang-ru { background: #8e44ad; }
        .lang-pl { background: #c0392b; }
        ...
        pre { background: #f5f5f5; padding: 1rem; border-radius: 4px; overflow-x: auto; font-size: 0.85rem; white-space: pre-wrap; word-wrap: break-word; }
        textarea { font-family: monospace; }
    </style>
</head>
<body>
    <header>
        <div class="logo"><a href="/">Tolkien KG</a></div>
        <nav>
            <a href="/">Home</a>
            <div class="dropdown">
...
<div class="filter-bar">
    ...
    <div class="filter-group"><label>Source:</label>
        <select id="source-filter">
            <option value="all">All</option>
            <option value="wiki">Wiki (Tolkien Gateway)</option>
            <option value="card">Cards (MECCG)</option>
            <option value="external">External (DBpedia/Wikidata)</option>
        </select>
    </div>
</div>
...
{% if outgoing %}
<div class="section">
    <div class="section-header">Properties <span class="tag tag-wiki">Wiki</span></div>
    <table>
